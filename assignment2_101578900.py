...
# 1. This dictionary stores common port numbers and their associated service names
common_ports = {
    21: "FTP",
    22: "SSH",
    23: "Telnet",
...
class NetworkTool:
    ...
    # 1.Using @property allows us to safely access a private variable without exposing it directly.
    # 2. It also lets us add validation logic when setting a value which ultimately helps prevent invalid data.
    # 3. The setter ensures that an empty string is not accepted as a target.
    # 4. Overall, this makes the code more secure and easy to manage.
...
# 1. PortScanner inherits from NetworkTool meaning it'll automatically gets access to the target attribute and its getter and setter methods. 
# 2. This prevents code duplication since we don’t need to rewrite the logic for handling the target IP.
# 3. PortScanner uses self.target which is managed by the parent class.
...
class PortScanner(NetworkTool):
    ...
    def scan_port(self, port):
        ...
        #     TODO: Your 2-4 sentence answer here... (
        # Part 2, Q4)

        # 1. Without try-except, any network-related error (like a timeout or unreachable host)
        # 2. would cause the program to crash immediately. This would stop the entire scan process.
        # 3. By using exception handling, we can catch errors and continue scanning other ports.
        # 4. This makes the program more stable and reliable.
    ...
    # 1. Threading allows multiple ports to be scanned at the same time instead of one by one.
    # 2. If we scanned 1024 ports sequentially, it would take a long time due to timeouts on each port.
    # 3. By using threads, many connections happen in parallel, which significantly speeds up the scan.
    # 4. This makes the scanner much more efficient.
...
# 1. One feature I would add is filtering results by a specific service name like HTTP or SSH.
# 2. This could be implemented using a list comprehension to return only matching services from scan_results.
# 3. This would make it easier for users to quickly find relevant open ports.
# 4. Diagram: See diagram_studentID.png in the repository root